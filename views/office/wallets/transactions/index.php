<div id="accordion">
    <?php echo $this->renderPartial('_filter', array(
		'list_alias_transaction'=>$list_alias_transaction,
			...
			<tbody>
				<? foreach ($modelsTransactions as $key => $modelTransaction) : ?>
					<tr>
						...
						<td><?=MSmarty::date_format($modelTransaction->date_open, 'd.m.Y')?> <?=MSmarty::date_format($modelTransaction->date_open, '%H:%M')?></td>
						<td>
							<? if ($modelTransaction->date_closed != NULL) : ?>
								<?=MSmarty::date_format($modelTransaction->date_closed, 'd.m.Y')?> <?=MSmarty::date_format($modelTransaction->date_closed, '%H:%M')?>
							<? endif; ?>
						</td>
					</tr>
				<? endforeach; ?>
