...
<input type="hidden" id="asseturl" value="<?=Yii::app()->assetManager->getPublishedUrl(Yii::getPathOfAlias('uploads.css'))?>">
<?php
 $options = array();
...
<p class="note">Поля с <span class="required">*</span> обязательно должны быть заполнены.</p>
    ...
    <?php foreach ($model_childs as $key => $value) {
        echo  CHtml::errorSummary(array($value), '');
     }?>
    <div class="pad"></div>
...
<table class="form">
    <tbody>
       ...
       <!--Загрузка превью--> 
        ...
        <?php if($model->isNewRecord) :?>
            <?php foreach ($model_childs as $key => $model_child):?>
                <tr class="file_load">
                    <td width="120" style="padding-top: 10px;"><label><?=CHtml::activeLabelEx($model_child,'name'); ?></label></td>
                    <td width="120"><?=CHtml::activeTextField($model_child,'[' . $key . ']name', array('class' => 'normal', 'size' => '0', 'style' => 'width: 250px;')); ?></td>
                    <td width="120" style="padding-top: 10px;"><label>Файл для загрузки *</label></td>
                    <td>
                        <?php echo CHtml::activeFileField($model_child, 'image[' . $key . ']', array('title'=>'Допустимые типы файлов - pdf, flv, png, jpg, jpeg, gif, tif, mp3, zip, rar, txt')); ?>
                        <?php if($key > 0):?><?php echo CHtml::image(Yii::app()->assetManager->getPublishedUrl(Yii::getPathOfAlias('uploads.css')) . '/css/trash-26.png', 'Удалить', array('class' => 'delmultifile', 'data-status' => 'new')); ?><?php endif;?>
                    </td>
                </tr>        
            <?php endforeach;?>
        <?php else:?>
                <tr><td colspan="3"><strong>Загруженные файлы:</strong></td></tr>
                <?php foreach ($model_childs as $key => $model_child):?>
                    <tr class="file_load">
                        <td width="120" style="padding-top: 10px;"><label><?=CHtml::activeLabelEx($model_child,'name'); ?></label></td>
                        <td width="120"><?=CHtml::activeTextField($model_child,'[' . $key . ']name', array('class' => 'normal', 'size' => '0', 'style' => 'width: 250px;')); ?></td>
                        ...
                        <td>
                            <?=!$model_child->hasErrors() ? $model_child->row_name : CHtml::activeFileField($model_child, 'image[' . $key . ']', array('title'=>'Допустимые типы файлов - pdf, flv, png, jpg, jpeg, gif, tif, mp3, zip, rar, txt'));?>
                            <?php echo CHtml::image(Yii::app()->assetManager->getPublishedUrl(Yii::getPathOfAlias('uploads.css')) . '/css/trash-26.png', 'Удалить', array('class' => 'delmultifile', 'data-status' => $model_child->id ? 'old' : 'new', 'data-id' => $model_child->id)); ?>
                        </td>
                    </tr>        
                <?php endforeach;?>
        ...
        <?php endif;?>
        <tr><td><button onclick="anotherfile(event);">Добавить файл</button></td></tr>
