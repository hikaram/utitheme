<? if ((Yii::app()->user->checkAccess('AdminPagesPageChangeMode')) && ((bool)PagesSettings::isAllowedChangeByAdmin())) : ?>
    <? if (PagesSettings::getEditMode() == PagesSettings::EDIT_MODE_PRO) : ?>
        <div class="alert alert-warning">
			<i class="fa fa-info mr10"></i>
			<?=Yii::t('app', 'Включен профессиональный режим редактора страниц. Вы можете включить упрощенный режим редактора.')?>
			<?=CHtml::link(Yii::t('app', 'Перейти в lite-режим'), $this->createUrl('changemode'), array('class' => 'alert-link'))?>
		</div>
    <? endif; ?>
<? endif; ?>
...
<? if(empty($all_pages)) :?>
	<div class="note note-success">
		...
		<? if (Yii::app()->user->checkAccess('AdminPagesPageCreate')) : ?>
			Вы можете 
			<?=CHtml::link(Yii::t('app', 'добавить страницу'), $this->createUrl('create'))?>
		<? endif; ?>
	</div>
<? else : ?>
					...
					<table class="table table-hover">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<? if ($page->theme == NULL) : ?><?=Yii::t('app', 'По умолчанию')?>
									<? else : ?><?=$page->theme->name?>
									<? endif; ?>
								</td>
								...
								<td style="white-space: nowrap; word-wrap: normal;"> 
									...
									<? if ((Yii::app()->user->checkAccess('AdminPagesPageEdit')) && ((Yii::app()->user->username == Yii::app()->params['superAdminInfo']['username']) || ($page->is_edit_admin))) : ?>
										<?=CHtml::link('<i class="glyphicon glyphicon-pencil"></i>', $this->createUrl('pro/edit', array('id' => $page->id)), array(
