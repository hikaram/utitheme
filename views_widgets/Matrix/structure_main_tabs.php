...
<div id="tabs">
    <ul>
        <? foreach ($list as $key => $data) : ?>
            <li id="tab_<?=$key?>"><a href="#tabs-<?=$key?>"><?=CHtml::encode($key + 1)?></a></li>
        <? endforeach; ?>
    </ul>
    ...
    <? foreach ($list as $key => $data) : ?>
        <div id="tabs-<?=$key?>">
            <div style="position: relative">
                <table class="matrix-table" style="float: none;" border="0" cellspacing="0" cellpadding="0">
                    <? foreach ($data as $width_level => $tr_value) : ?>
                        <tr>
                            <? foreach ($tr_value as $depth_level => $token) : ?>
                                <td class="text-center" colspan="<?=count($data[$depth - 1])/count($tr_value)?>">
                                    <? if (!$token->empty) : ?>
